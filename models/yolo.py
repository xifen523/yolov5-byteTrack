    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                y = x[i].sigmoid()
                if True:  # if self.inplace:
                    y[..., 0:2] = (y[..., 0:2] * 2 + self.grid[i]) * self.stride[i]  # xy
                    y[..., 2:4] = (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i]  # wh
                    y[..., 4:] = (y[..., 4:] * 1.02041 - 0.010205).clamp(0.0, 1.0)
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [
            Conv, Scale1, Scale2, Scale3, Scale5, Scale7, Scale8, GhostConv, Bottleneck, Bottleneck_alpha,
            GhostBottleneck, SPP, SPPF, C2SPPF, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP, C2b, C2a, C2c,
            C3, C3a, C3b, C3c, C3d, C3F, C3TR, C3SPP, C3Ghost, RepVGGBlock]:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in [BottleneckCSP, C2b, C2a, C2c, C3, C3a, C3b, C3c, C3d, C3F, C3TR, C3Ghost]:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='yolov5l6-7b-C3c.yaml', help='model.yaml')
