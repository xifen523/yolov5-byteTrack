...
class Scale5(nn.Module):
    # Scale down by 2x in width and height
    def __init__(self, c1, c2, k=5, s=2):  # ch_in, ch_out, shortcut, groups, expansion
        super().__init__()
        c_ = (c1 + c2) // 2
...
class C3a(nn.Module):
    ...
    def forward(self, x):
        y = self.cv1(x)
        return self.cv3(torch.cat((self.m(y), self.cv2(y)), 1))
...
class C3b(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.cv3(torch.cat((self.m(y), y), 1))
...
class C3c(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv3 = Conv(c1, c1, 1)  # optional act=FReLU(c2)
        self.m = nn.Sequential(*(CrossConv(c_, c_, 3, 1, g, 1.0, shortcut) for _ in range(n)))
...
class C3d(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        c_ = int(c2 * e)  # hidden channels
        c__ = int(c1 * 0.75)
        self.cv1 = Conv(c__, c_, 1, 1)
        self.cv2 = Conv(c__, c_, 1, 1)
        self.cv3 = Conv(c1, c__, 1)  # optional act=FReLU(c2)
        self.m = nn.Sequential(*(CrossConv(c_, c_, 3, 1, g, 1.0, shortcut) for _ in range(n)))
